...
onload=function(){
	...
	function createData(){
		...
		let nodes= [];
		for(let i=1;i<=sz;i++){
			nodes.push({id:1, label:"Person "+i})
		}
		nodes= new vis.DataSet(nodes);

		//Did this to dynamically create edges with random amount 
		const edges = [];
		for(let i=1;i<=sz;i++){
			for(let j=i+1;j<=sz;j++){
				//edges should not be one directioanl so putting this Math.random
				if(Math.random()>0.5){
					if(Math.random()>0.5){
	...
	function solveData(data){
		...
		for(let i=0;data['edges'].length;i++){
			const egde=data['edges'][i];
